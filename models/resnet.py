...
# ###############################################################################
# # My CBAM Functions
# # change:在BasicBlock和Bottleneck中加入SpatialAttention和ChannelAttention，并使用 model.load_state_dict(state_dict,strict=False)加载预训练权重
# # version 2
# # name: checkpoints_1022_CBAMpre
# # 2023.10.22
# ###############################################################################
#
...
#
#         # attention
#         atten = self.channel_att(out) * out
#         atten = self.spatial_att(atten) * atten
#
...
#
# # #######################################################################
# # My Resnet18 + SE Model
# # 加SE的BasicBlock和Bottleneck
#
# # #######################################################################
# # SE
# class SE(nn.Module):
...
#     def forward(self, x):
#         b, c, _, _ = x.size()
#         y = self.avgpool(x).view(b, c)  # squeeze擦欧洲哦
#         y = self.l1(y)
#         y = self.relu(y)
#         y = self.l2(y)
#         y = self.sig(y)
#         y = y.view(b, c, 1, 1)
#         return x * y.expand_as(x)   # 注意力作用于每一个通道上
#
# class BasicBlock(nn.Module):
#     expansion = 1
#
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))  # 指定输出的目标大小
